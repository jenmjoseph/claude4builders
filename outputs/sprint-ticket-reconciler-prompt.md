...
**Claude Code** (with Jira MCP connected) or a **Claude Project workspace** preloaded with sprint context.

**Required:** `sprint-ticket-reconciler` skill installed at `~/.claude/skills/sprint-ticket-reconciler/`
...
**Run the Sprint Ticket Reconciler skill** — work through the following steps in order for this block:

**Step 1 — Validate**
Check if the block is red (proceed) or green (stop). Extract the block title, duration, and any ticket reference embedded in the title.
...
Write story points, status, sprint membership, and description to Jira via Jira MCP. Confirm each update. Then remind me to flip the calendar block from red to green.
...
8. **Block processed** — reminder to flip block green
